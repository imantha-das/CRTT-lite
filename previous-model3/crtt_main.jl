...
@doc """Function to plot casualty attributes through time"""->
function plot_casualty_attributes(track_attr::Dict,ticks::Int)
    Plots.plot()
    anim = @animate for t = 1:ticks 
        for k in keys(track_attr)
            Plots.bar!(
                [String(k)],
                track_attr[k][t:t],
                label = String(k)
...
anim = plot_casualty_attributes(track_cas_attr_ts3, 500)
gif(anim, fps =40)
p = Plots.plot()
Plots.bar!(
    [:x,:y],
...
begin
    p = Plots.plot()
    Plots.bar!(
        repeat(["awaiting_rescue"],2),
        track_cas_attr_ts3[:awaiting_rescue][1:2]
    )
end

track_cas_attr_ts3[:in_stabilize_q]
track_cas_attr_ts3[:in_hosp_q]